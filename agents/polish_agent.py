...
class PolishAgent(BaseAgent):
    ...
    def __init__(self, **kwargs):
        ...
        self.image_gen_model_name = (
            self.exp_config.image_gen_model_name
        )  # e.g., gemini-3-pro-image-preview
        self.main_model_name = (
            self.exp_config.main_model_name
        ...
        if self.exp_config.task_name == "plot":
            self.style_guide_filename = f"{style_prefix}_plot_style_guide.md"
            self.suggestion_system_prompt = PLOT_SUGGESTION_SYSTEM_PROMPT
            self.task_config = {
                "task_name": "plot",
            }
        else:
            self.style_guide_filename = f"{style_prefix}_diagram_style_guide.md"
    ...
    async def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not gt_image_path_rel:
            print("⚠️  No GT image path found in data")
            return data

        # Resolve relative path using work_dir
        gt_image_path = (
            self.exp_config.work_dir
            / f"data/PaperBananaBench/{task_name}"
            / gt_image_path_rel
        )
        ...
        # Load style guide
        style_guide_path = (
            self.exp_config.work_dir / "style_guides" / self.style_guide_filename
        )
        try:
        ...
        if not suggestions or "No changes needed" in suggestions:
            print("✨ No changes needed for this image.")
        ...
        print("🎨 [Step 2] Polishing image with suggestions...")
