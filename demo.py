...
st.set_page_config(
    layout="wide", page_title="PaperBanana Parallel Demo", page_icon="🍌"
)
...
async def refine_image_with_nanoviz(
    ...
    try:
        ...
        return (
            None,
            "❌ Error: google-genai SDK not installed and OpenRouter unavailable.",
        )
    ...
    if google_api_key:
        ...
        return (
            None,
            "❌ Error: No API credentials configured. Set OPENROUTER_API_KEY, GOOGLE_API_KEY, or configure Vertex AI project in configs/model_config.yaml.",
        )
...
def display_candidate_result(result, candidate_id, exp_mode):
    ...
    if final_image_key and final_image_key in result:
        ...
        if img:
            st.image(
                ...
                caption=f"Candidate {candidate_id} (Final)",
            )
    ...
    if len(stages) > 1:
        with st.expander(
            f"🔄 View Evolution Timeline ({len(stages)} stages)", expanded=False
        ):
...
def main():
    ...
    with tab1:
        st.markdown(
            "### Generate multiple diagram candidates from your method section and caption"
        )
        ...
        with st.sidebar:
            ...
            if default_model in text_model_presets:
                text_model_presets.remove(default_model)
            if default_model:
                text_model_presets.insert(0, default_model)
            ...
            image_model_presets = [
                "openai/gpt-5.4-image-2",  # Stack B/C — Arena #1 image, best text rendering
                "google/gemini-3-pro-image-preview",  # Stack A — Nano Banana Pro
                "google/gemini-3.1-flash-image-preview",  # Cheap fallback — Nano Banana 2
            ]
            if default_image_model in image_model_presets:
                image_model_presets.remove(default_image_model)
            if default_image_model:
                image_model_presets.insert(0, default_image_model)
...
The framework extends to statistical plots by adjusting the Visualizer and Critic agents. For numerical precision, the Visualizer converts the description $P_t$ into executable Python Matplotlib code: $I_t = \text{VLM}_{\text{code}}(P_t)$. The Critic evaluates the rendered plot and generates a refined description $P_{t+1}$ addressing inaccuracies or imperfections: $P_{t+1} = \text{VLM}_{\text{critic}}(I_t, S, C, P_t)$. The same $T=3$ round iterative refinement process applies. While we prioritize this code-based approach for accuracy, we also explore direct image generation in Section \ref{sec:discussion}. See Appendix \ref{app_sec:plot_agent_prompt} for adjusted prompts."""
        ...
        # Process button
        if st.button(
            "🚀 Generate Candidates", type="primary", use_container_width=True
        ):
            if not method_content or not caption:
                ...
                with st.spinner(
                    f"Generating {num_candidates} candidates in parallel... This may take a few minutes."
                ):
                    ...
                    try:
                        ...
                        try:
                            ...
                            st.success(
                                f"✅ Successfully generated {len(results)} candidates!"
                            )
                            st.info(f"💾 Results saved to: `{json_filename.name}`")
                        except Exception as e:
                            st.warning(
                                f"⚠️ Generated {len(results)} candidates, but failed to save JSON: {e}"
                            )
                    except Exception as e:
        ...
        if "results" in st.session_state and st.session_state["results"]:
            ...
            st.caption(
                f"Generated at: {timestamp} | Pipeline: {mode_info.get(current_mode, current_mode)}"
            )
            ...
            if "json_file" in st.session_state:
                ...
                if json_file_path.exists():
                    ...
                    with col1:
                        st.info(
                            f"📄 Results saved to: `{json_file_path.relative_to(Path.cwd())}`"
                        )
                    with col2:
                    ...
                    if result_idx < num_results:
                        with cols[col_idx]:
                            display_candidate_result(
                                results[result_idx], result_idx, current_mode
                            )
                ...
                with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
                    ...
                    for candidate_id, result in enumerate(results):
                        ...
                        for round_idx in range(3, -1, -1):
                            image_key = (
                                f"target_{task_name}_critic_desc{round_idx}_base64_jpg"
                            )
                            if image_key in result and result[image_key]:
                                final_image_key = image_key
                                break
    ...
    with tab2:
        ...
        st.caption(
            "Upload an image from the candidates or any diagram, describe changes, and generate a high-res version"
        )
        ...
        if uploaded_file is not None:
            ...
            with col2:
                ...
                if st.button(
                    "✨ Refine Image", type="primary", use_container_width=True
                ):
                    if not edit_prompt:
                        st.error("Please provide edit instructions!")
                    else:
                        with st.spinner(
                            f"Refining image to {refine_resolution} resolution... This may take a minute."
                        ):
                            try:
                                ...
                                if refined_bytes:
                                    ...
                                    st.session_state["refine_timestamp"] = (
            ...
            if "refined_image" in st.session_state:
                ...
                st.caption(
                    f"Generated at: {st.session_state.get('refine_timestamp', 'N/A')} | Resolution: {refine_resolution}"
                )
                ...
                with col2:
                    ...
                    refined_image = Image.open(
                        BytesIO(st.session_state["refined_image"])
                    )
